...
const updateRecipe =()=>{
  navigate(`/update/details/${recipe.id}`);
}
useEffect(()=>{
  return()=>{
  }
  ...
  return recipe ? (
    ...
    <div className="container mx-auto px-6 max-w-4xl">
      <div className="bg-white p-8 rounded-xl shadow-md">
        <img src={recipedata.image} alt="Recipe Image" className="rounded-md w-full mb-6"/>

        <h2 className="text-3xl font-bold text-gray-800 mb-4">{recipedata.title}</h2>
        ...
        <div className="flex space-x-4">
          <button onClick={updateRecipe} className="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-6 rounded-md transition">Update</button>
          <button onClick={DeleteHandler} className="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-md transition">Delete</button>
        </div>
      </div>
    </div>
  </section>
